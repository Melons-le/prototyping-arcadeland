    ...
    public class AirplaneController : MonoBehaviour, IVehicleStartLine<Vector3>, IGyroStartLine<Quaternion>
    {
        ...
        void PlaneMovement()
        {
            ...
            if (b_Down)
            {
                ...
                if (!vehicleAI.enabled) smoothGampadMove = vehicleInputs.ReturnDownInputValue();    // When player use Gamepad movement depend of the value of the axis. Create smoother movement.
            ...
            else if (b_Up)
            {
                ...
                upDownRotSpeed = Mathf.MoveTowards(upDownRotSpeed, 1, Time.deltaTime);
                float smoothGampadMove = 1;
                if (!vehicleAI.enabled) smoothGampadMove = vehicleInputs.ReturnUpInputValue();    // When player use Gamepad movement depend of the value of the axis. Create smoother movement.
            ...
            if (b_Left)
            {
                ...
                //offsetRotSpeed = Mathf.MoveTowards(offsetRotSpeed, offsetRotSpeedTarget, Time.deltaTime * offsetRotSpeedReachTarget);

                rotationSpeed = Mathf.MoveTowards(rotationSpeed, 300, Time.deltaTime * 300 * animCurvesmoothUDLR.Evaluate(curveSpeed));

                float smoothGampadMove = 1;
                if (!vehicleAI.enabled) vehicleInputs.ReturnLeftInputValue();    // When player use Gamepad movement depend of the value of the axis. Create smoother movement.
            ...
            else if (b_Right)
            {
                ...
                float smoothGampadMove = 1;
                if (!vehicleAI.enabled) smoothGampadMove = vehicleInputs.ReturnRightInputValue();    // When player use Gamepad movement depend of the value of the axis. Create smoother movement.
                newXRotation = Mathf.MoveTowards(newXRotation, 1/* + offsetRotSpeed*/, Time.deltaTime * rotationSpeed * smoothGampadMove);
